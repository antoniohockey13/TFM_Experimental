...
def plot_tot_calibrated(file, canvas_name, title, file_name, folder, cut=""):
    c = ROOT.TCanvas(canvas_name, canvas_name)
    file.Hits.Draw("tot_cal>>tot_calibrated(75,0,10)", cut,"")
...
def plot_toa_calibrated(file, canvas_name, title, file_name, folder, cut=""):
    c = ROOT.TCanvas(canvas_name, canvas_name)
    c.SetLeftMargin(0.2)
    file.Hits.Draw("toa_cal>>toa_calibrated(125,-0.5,14)", cut,"")
    ...
    toa_calibrated.GetYaxis().SetTitleOffset(1.5)
    c.Draw()
    if save_plots:
        c.SaveAs(f"Pictures/{folder}/{file_name}{FORMAT}")
    if not omit_plots:
...
def plot_cal_log(file, canvas_name, title, file_name, folder, cut=""):
    c = ROOT.TCanvas(canvas_name, canvas_name)
    c.SetLogy()
...
def plot_toa_calibration(file, canvas_name, title, file_name, folder, cut=""):
    c = ROOT.TCanvas(canvas_name, canvas_name)
    file.Hits.Draw("toa_cal:cal>>toa_cal(125,0,14,1024,0,1023)", cut,"colz")
    toa_cal = ROOT.gDirectory.Get("toa_cal")
    toa_cal.SetTitle(title)
    toa_cal.GetXaxis().SetTitle("ToA/ns")
    toa_cal.GetYaxis().SetTitle("Calibration")
    c.Draw()
    if save_plots:
        c.SaveAs(f"Pictures/{folder}/{file_name}{FORMAT}")
    if not omit_plots:
